...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _selectedColor = ApplicationMainColor;
    }
    return self;
}
...
- (void)setSelectedColor:(UIColor *)selectedColor {
    ...
    selectionImageView.backgroundColor = selectedColor;
}

- (void)createContentTableView {
    
    selectionImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[self selectedImageName]]];
    _selectionRect = [self.dataSource rectForSelectionInSelector:self];
    selectionImageView.frame = _selectionRect;
    selectionImageView.backgroundColor = self.selectedColor;
    selectionImageView.layer.cornerRadius = 3;
//    selectionImageView.layer.borderColor = [UIColor blackColor].CGColor;
//    selectionImageView.layer.borderWidth = 1.0;
